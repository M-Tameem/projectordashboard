...
- Weather is requested every 15 minutes. Aircraft/ISS data syncs every 15 seconds while Overhead is visible and every 45 seconds otherwise. If the primary aircraft feed fails, the primary is still retried every 45 seconds but airplanes.live is fetched at most once every three minutes (480 requests/day maximum) and its intermediate samples are extrapolated locally. Motion between samples makes no extra requests. With no saved location, no ambient network requests run. Planet and bright-star positions are tiny on-device calculations refreshed from the current clock.
