...
- **Tiles** launch websites or programs on the active target. In normal mode that is the assigned projector; in **Tablet only** mode it is the tablet itself. Once a tile's app window is found, the tile changes to **ON** with an accent outline. Tapping that tile again restores and brings the same window to the front instead of opening another Supermium window or app instance; the state clears automatically when that window closes. A stable per-tile window tag lets the dashboard recover that relationship after its own theme or display-mode restart as long as the launched app remains open. Website tiles always use **Supermium** and default to borderless fullscreen; Settings > Projector can switch launches to normal framed maximized windows. Twitch, Caedrel, and Miruro are included automatically, and ordinary web tiles cache their site icon locally after the first successful download.
- **Weather + Sky now** use one manually saved location rather than Windows Location Services, which is unreliable on the target Windows 8.1 tablet. Open **Settings > Ambient** once, search a city/town/postal code, optionally refine the exact latitude/longitude, and set the true compass heading toward the physical top edge of the projected image. If that edge is above your head, use the direction your head points; if it is above your feet, use the direction your feet point. Weather and next sunrise come from Open-Meteo; aircraft use adsb.lol with airplanes.live as an automatic fallback; the ISS orbital elements come from Where the ISS at?; planet and bright-star bearings are calculated locally. None requires an API key, account, subscription, or paid service. The **Live data** control on the main Sky Now strip (also available under Settings > Ambient) immediately cancels aircraft/ISS requests and prevents further Sky Now polling until it is turned back on; local airports, planets, stars, and a recently cached ISS pass estimate remain available, and the separate weather schedule is unchanged. Bearings include both true compass direction and a view-relative cue such as `NW · left`.
- **Overhead** is a first-class action on the main screen: **Project sky** sends it straight to the projector by default, while systems without an available projector open the tablet view. It is a live ceiling view seen from below, so east/west are intentionally mirrored from a ground map and the complete 360° compass wraps around the center according to the saved display heading. Celestial objects remain a literal full-sky hemisphere: center is zenith (90°) and all four outer edges are horizon (0°). Aircraft use the separate selected-distance ruler: center is 0 km and the marked inner limit is the chosen search radius, so an aircraft 60 km away in a 120 km view renders halfway to that limit instead of being crushed against the horizon. Aircraft silhouettes show callsign, type, registration, course, speed, altitude, distance, and viewing elevation. A forward-only dashed line shows reported course; brighter solid segments retain accepted observed movement. Stale, out-of-order, implausibly jumping, and unexplained reversing ADS-B samples are rejected, and extrapolation stops after 75 seconds. The **Range** control selects 20–460 km in 20 km steps; `−` and `+` only change a pending value, and **Done** saves it and makes one refresh. The normal 40 km view keeps the 5° clutter cutoff; expanded views admit visible aircraft down to 0.5° above the curved-earth horizon. Up to five nearby IATA-coded airports appear as distance-labelled horizon-edge markers and can be hidden with **Airports: Off**; the bundled catalog is a compact snapshot derived from [OurAirports public-domain data](https://ourairports.com/data/) and never makes a web request. The dashboard caches one current ISS TLE for six hours, propagates that orbital model locally, and shows an estimated next horizon crossing, local clock time, and peak elevation without position polling. The ISS also has a compact azimuth/elevation/range label and observed pass trail. Planet dots are now directional arrows based on their calculated position ten minutes ahead, while major stars continue moving from local clock-based calculations. Fixed horizon, elevation, zenith, and ruler graphics use a neutral instrument color rather than the live-aircraft accent.
- **Color** under **Tools** cycles Cyan, Amber, Violet, and Emerald palettes. The saved palette applies immediately to the entire tablet UI, settings, overlays, projector idle screen, and live sky map.
- **Remote** opens the same large-button touch remote in either display mode. It controls the current app with Left, Right, Space/play-pause, Escape/back, F/fullscreen, Enter, output Volume −/+, Mute, and Keyboard buttons. The volume actions adjust the selected output endpoint directly rather than requiring a precise slider drag.
- **Tablet only** under Settings > Projector restarts into a complete one-screen workflow without deleting the saved projector assignment. Fullscreen apps open on the tablet and a compact always-on-top **Dashboard / Remote / Screen off / OFF** overlay lets you return, open the shared remote, sleep the panel, or trigger the locked emergency shutdown without a keyboard or mouse. **Screen off** sends Windows' monitor-power command after the activating touch is released: Supermium, playback, audio, timers, and network activity continue normally, and touching the screen wakes the panel. The same control appears in the dashboard header and Projector settings. **Hide** remains separate—it puts the fullscreen app away without closing its tabs.
- **Preview** opens a live view of the current projector app on the dashboard while leaving volume, brightness, and the TouchMousePointer area available. **Fullscreen on tablet** expands that same live surface across the tablet, so the tablet and projector show one shared Supermium window—not two tabs, decoders, sessions, or audio streams. The small Dashboard / Remote / Screen off / OFF overlay remains available in fullscreen preview. Windows creates this as [a dynamic DWM source-to-destination relationship](https://learn.microsoft.com/en-us/windows/win32/dwm/thumbnail-ovw) rather than a screenshot loop. Protected/DRM video may deliberately render black in the preview even while ordinary pages and video continue working on the projector.
...
- The movable Windows on-screen keyboard now opens automatically when a dashboard text box receives focus or when Windows reports a text caret in the foreground app on the active screen. It is placed to the left of the TouchMousePointer corner. The Remote and **Tools > Keyboard** buttons remain one-tap fallbacks, and the Windows touch keyboard is used if OSK cannot start. **Tools > Windows desktop** minimizes the dashboard when you genuinely need Windows; tap it in the taskbar to come back.
- **Tools > Daily tablet auto-lock** uses the same no-typing Hour, 5 Minute, and AM/PM controls. At the enabled time it locks the tablet exactly like Win+L. Windows still requires the normal sign-in to unlock; the dashboard does not store or bypass a password.
...
- The daily **Alarm** is configured without typing: large Hour −/+, 5 Minute −/+, and AM/PM buttons save immediately. A safe built-in Speakers endpoint must be selected before it can be enabled. While ringing, the dashboard switches to that tablet endpoint, forces it to 100% and unmuted, mutes other application/browser sessions, then restores the prior default output and every saved volume/mute state on Dismiss or Snooze. Bluetooth, USB, HDMI, headphones, and projector endpoints are rejected for alarm use. The alarm runs only while the dashboard and tablet are awake.
...
src\AirportCatalog.cs     compressed offline IATA airport coordinates
...
src\ReturnOverlayWindow.cs tablet-only return / remote / screen-off / emergency overlay
src\RemoteControlWindow.cs shared large-button current-app remote
...
- Weather is requested every 15 minutes. Aircraft data syncs every 15 seconds while Overhead is visible and every 45 seconds otherwise. The ISS uses a single [Where the ISS at? TLE request](https://wheretheiss.at/w/developer) at most once every six hours; current position and the next 36 hours of horizon passes are then propagated locally. Turning **Live data** off cancels an in-flight Sky Now request, prevents future aircraft/TLE polling, clears remote tracks, and retains offline airport markers, local planet/star calculations, and a cached TLE pass estimate for up to 24 hours; weather remains separately enabled. If the primary aircraft feed fails, the primary is still retried every 45 seconds but airplanes.live is fetched at most once every three minutes (480 requests/day maximum). Cached ADS-B positions older than 75 seconds are no longer rendered or extrapolated. Motion between samples makes no extra requests. With no saved location, no ambient network requests run. Airport lookup, planet/star motion, and pass prediction are small on-device calculations.
