...
- **Weather + Sky now** use one manually saved location rather than Windows Location Services, which is unreliable on the target Windows 8.1 tablet. Open **Settings > Ambient** once, search a city/town/postal code, optionally refine the exact latitude/longitude, and set the true compass heading faced by the top of the display. Weather comes from Open-Meteo; nearby aircraft come from adsb.lol; the ISS position comes from Where the ISS at?; planet bearings are calculated locally from JPL orbital elements. None requires an API key, account, subscription, or paid service. Bearings include both true compass direction and a view-relative cue such as `NW · left`.
...
- **Idle** hides the open projector apps without closing their windows or browser tabs and shows the ambient clock, weather, ISS/planet status, and nearby-aircraft count. **Wake** restores those same windows.
...
src\AmbientData.cs        free weather/aircraft/ISS clients + local planet positions
...
- No animations anywhere; the recurring local work is a 1 Hz clock/alarm/auto-lock check and a lightweight audio-device refresh every five seconds.
- Weather is requested every 15 minutes and the small aircraft/ISS snapshot every 45 seconds, asynchronously. With no saved location, no ambient network requests run. Planet positions are a tiny on-device calculation.
